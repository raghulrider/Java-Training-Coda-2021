	...
	<div class="container">
		...
		<table class="center pure-table">
			...
			<tbody>
				...
				<c:set var="items" scope="session" value="${sessionScope.selectedItems}" />
				<c:set var="total" value="${0}" />
				<c:set var="count" value="${1}" />
				<c:forEach items="${items}" var="item">
					<c:set var="localTotal" value="${0}" />
					<c:set var="localTotal" value="${item.key.getPrice()*item.value}" />
					<c:set var="total" value="${total+localTotal}" />

					<tr>
						<td><c:out value="${count}" /></td>
						<td><c:out value="${item.key.getItemid()}" /></td>
						<td><c:out value="${item.key.getItemname()}" /></td>
						<td><c:out value="${item.key.getUnit()}" /></td>
						<td>₹<c:out value="${item.key.getPrice()}" />
						</td>
